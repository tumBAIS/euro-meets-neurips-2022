...
void Population::doLocalSearchAndAddIndividual(Individual* indiv) {
    ...
    // infeasibilities (w.r.t. capacities and time warps) in a new Local Search in case of
    // infeasibility
    if (!indiv->isFeasible
...
void Population::generatePopulation() {
    if (params->getNumberOfPossibleClients() <= 1) {
    ...
    for (int i = 0; i < nofNearestIndividualsToGenerate; i++) {
        ...
        localSearch->constructIndividualWithSeedOrder(toleratedCapacityViolation, toleratedTimeWarp,
                                                      false, &indiv);
    ...
    for (int i = 0; i < nofSweepIndividualsToGenerate; i++) {
        ...
        int fillPercentage
            = i == 0 ? 100
                     : minSweepFillPercentage + params->rng() % (100 - minSweepFillPercentage + 1);
        Individual indiv(params, false);
...
void Population::removeWorstBiasedFitness(SubPopulation& pop) {
    ...
    for (int i = 1; i < static_cast<int>(pop.size()); i++) {
        ...
        // Accept new worst solution if we found a clone and the current worst is not a clone, or
        // both are clones/no clones and fitness is worse
...
void Population::restart(bool recreate) {
    if (!params->config.quiet) {
    ...
    // Clear the pools of solutions and make a new empty individual as the best solution after the
    // restart
...
void Population::managePenalties() {
    ...
    if (fractionFeasibleLoad <= 0.01 && params->config.penaltyBooster > 0.
        ...
        params->penaltyCapacity
            = std::min(params->penaltyCapacity * params->config.penaltyBooster, 100000.);
    ...
    if (fractionFeasibleTimeWarp <= 0.01 && params->config.penaltyBooster > 0.
        ...
        params->penaltyTimeWarp
            = std::min(params->penaltyTimeWarp * params->config.penaltyBooster, 100000.);
    ...
    for (int i = 0; i < static_cast<int>(infeasibleSubpopulation.size()); i++) {
        infeasibleSubpopulation[i]->myCostSol.penalizedCost
            = infeasibleSubpopulation[i]->myCostSol.cost
...
Individual* Population::getBinaryTournament(bool updateRanks) {
...
std::pair<Individual*, Individual*> Population::getNonIdenticalParentsBinaryTournament() {
    static constexpr int MAX_NUM_TRIES = 20;
    // Pick two individual using a binary tournament
    Individual* parentA = getBinaryTournament(true);
    Individual* parentB = getBinaryTournament(false);
    int num_tries = 1;
    // Pick two other individuals as long as they are identical (try at most 19 times)
    while (parentA->brokenPairsDistance(parentB) < MY_EPSILON && num_tries < MAX_NUM_TRIES) {
        parentB = getBinaryTournament(false);
        num_tries++;
    }
    ...
    // Randomly pick one that is different
    std::vector<size_t> individual_indicies(feasibleSubpopulation.size()
                                            + infeasibleSubpopulation.size());
    std::iota(individual_indicies.begin(), individual_indicies.end(), 0);
    std::shuffle(individual_indicies.begin(), individual_indicies.end(), params->rng);
    for (auto index : individual_indicies) {
        parentB = (index < feasibleSubpopulation.size())
                      ? feasibleSubpopulation[index]
                      : infeasibleSubpopulation[index - feasibleSubpopulation.size()];
        if (parentA->brokenPairsDistance(parentB) >= MY_EPSILON) break;
    }
...
void Population::printState(int nbIter, int nbIterNoImprovement) {
    ...
    // of the last 100 solutions generated by LS, and the penalties for the capacit and the time
    // warp
...
void Population::logSolution(int nbIter, std::ofstream& myfile, Individual* indiv) {
    ...
    myfile << indiv->myCostSol.cost << ";" << indiv->myCostSol.capacityExcess << ";"
...
Population::Population(Params* params, LocalSearch* localSearch, bool generate)
